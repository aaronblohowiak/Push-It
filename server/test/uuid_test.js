...
test["uuid should return only 0-9a-zA-Z\-_ so we can have url-safe uuids"] = function(assert){
  var guid = uuid(),
...
test["uuid should return 22 chars by default"] = function(assert){
  var guid = uuid(),
      len = guid.length;
  assert.equal(len, 22, guid + " should be 22 chars long, it was "+len);
};

test["uuid should respect a length param"] = function(assert){
  var guid = uuid(10);
  assert.equal(guid.length, 10, guid + "should be 10 chars long");
};

test["uuid should be respect a 'base' param"] = function(assert){
